...
static void encoder_thread(void *arg1, void *arg2, void *arg3)
{
	...
	// modified
	struct net_buf *audio_frame_out[2] = {NULL, NULL};
	...
	// ============================================================================================
		...
		// ================================== modified code ===========================================
			...
			struct audio_metadata *meta_in =
				net_buf_user_data(audio_frame_in);
			/*
			 * ===============================
			 * BIG 0 : Korean (LEFT)
			 * ===============================
			...
			audio_frame_out[0] = net_buf_alloc(&audio_q_enc_pool, K_NO_WAIT);


			if (audio_frame_out[0] == NULL) {
				LOG_WRN("No encoder buffer BIG0");
				net_buf_unref(audio_frame_in);
				continue;
			}


			struct audio_metadata *meta_out0 = net_buf_user_data(audio_frame_out[0]);


			*meta_out0 = *meta_in;


			meta_out0->data_coding = LC3;
			meta_out0->sample_rate_hz =
				sw_codec_cfg.encoder.sample_rate_hz;
			meta_out0->bitrate_bps =
				sw_codec_cfg.encoder.bitrate;

			meta_out0->bytes_per_location =
				(meta_out0->bitrate_bps *
				 meta_out0->data_len_us) / 8000000;

			meta_out0->interleaved = false;


			// 수정 2: BIG0은 LEFT만 사용
			meta_out0->locations = BT_AUDIO_LOCATION_FRONT_LEFT;
			...
			ret = sw_codec_encode(audio_frame_in, audio_frame_out[0]);
			ERR_CHK_MSG(ret, "BIG0 encode failed");
			...
			audio_frame_out[1] = net_buf_alloc(&audio_q_enc_pool, K_NO_WAIT);


			if (audio_frame_out[1] == NULL) {
				LOG_WRN("No encoder buffer BIG1");
				net_buf_unref(audio_frame_out[0]);
				net_buf_unref(audio_frame_in);
				continue;
			}
			...
			struct audio_metadata *meta_out1 =
				net_buf_user_data(audio_frame_out[1]);


			*meta_out1 = *meta_in;


			meta_out1->data_coding = LC3;
			meta_out1->sample_rate_hz =
				sw_codec_cfg.encoder.sample_rate_hz;

			meta_out1->bitrate_bps =
				sw_codec_cfg.encoder.bitrate;


			meta_out1->bytes_per_location =
				(meta_out1->bitrate_bps *
				 meta_out1->data_len_us) / 8000000;


			meta_out1->interleaved = false;


			// 수정 3: BIG1은 RIGHT만 사용
			meta_out1->locations = BT_AUDIO_LOCATION_FRONT_RIGHT;


			ret = sw_codec_encode(audio_frame_in, audio_frame_out[1]);
			ERR_CHK_MSG(ret, "BIG1 encode failed");
	// ============================================================================================
		...
		if (sw_codec_cfg.encoder.enabled) {
			// original code
			// streamctrl_send(audio_frame_out, 0);

			// net_buf_unref(audio_frame_out);


			// modified code
			stream_frame_send(audio_frame_out[0], audio_frame_out[1]);

			net_buf_unref(audio_frame_out[0]);
			net_buf_unref(audio_frame_out[1]);
		}
